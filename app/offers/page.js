...
import { getEldoradoOffers, pauseEldoradoOffer, resumeEldoradoOffer, deleteEldoradoOffer, updateEldoradoOfferPrice, updateEldoradoOfferDetails, bulkPauseEldoradoOffers, bulkDeleteEldoradoOffers } from "@/app/actions";
...
import { SearchIcon, FilterIcon, PackageIcon, Gamepad2Icon, ClockIcon, DollarSign, PlayCircleIcon, InfoIcon, HistoryIcon, PencilIcon, Trash2Icon, PauseCircleIcon, CheckSquareIcon, SquareIcon, MinusSquareIcon, ChevronLeftIcon, ChevronRightIcon, ArrowUpDownIcon, XIcon, CheckIcon, Loader2Icon, TagIcon, LayersIcon, TrendingUpIcon, ShoppingCartIcon, ImageIcon, GlobeIcon, CalendarIcon, HashIcon, PercentIcon, SlidersHorizontalIcon, ChevronDownIcon, PauseIcon, RefreshCwIcon } from "lucide-react";
...
const EditableField = ({ label, value, icon, onSave, type = "text", isLoading = false }) => {
    ...
    useEffect(() => {
        const timeoutId = setTimeout(() => setEditValue(value), 0);
...
const SelectFilter = ({ value, onChange, options, icon, className = "" }) => {
    ...
    useEffect(() => {
        if (!open) {
            const timeoutId = setTimeout(() => setSearch(""), 0);
            return () => clearTimeout(timeoutId);
        }
    }, [open]);
...
export default function OffersPage() {
    ...
    }, [searchQuery, offerStateFilter, categoryFilter, deliveryTimeFilter, pageIndex, pageSize, sortBy, sortAsc, activeOfferId]);
                                            ...
                                            <CardTitle className="flex items-start justify-between text-sm font-bold text-zinc-200">
                                                <div className="flex max-w-[65%] items-center gap-2">
                                                    {selectMode ? (
                                                        ...
                                                        <LightboxImage src={`https://fileserviceusprod.blob.core.windows.net/offerimages/${offer.mainOfferImage.smallImage}`} alt="Offer" className="h-10 w-10 shrink-0 rounded-md border border-zinc-800 object-cover" />
                                                    ) : (
                                                    ...
                                                    <div className="flex flex-col gap-0.5 truncate">
                                                        {offer.gameId && (
                                                            <div className="flex items-center gap-1">
                                                                <LightboxImage src={`https://assetsdelivery.eldorado.gg/v7/_assets_/icons/v28/${offer.gameId}.png`} alt="Game" className="h-3 w-3 shrink-0 rounded-sm object-cover opacity-60" />
                                ...
                                <div className="relative flex shrink-0 items-center justify-between overflow-hidden border-b border-zinc-800/50 bg-black/40 px-5 py-4 shadow-lg backdrop-blur-xl">
                                    ...
                                    <div className="relative z-10 flex-1 overflow-hidden">
                                        <h1 className="flex items-center gap-2.5 text-lg font-bold text-white">
                                            <div className="bg-primary/20 border-primary/30 shrink-0 rounded-lg border p-1.5 shadow-[0_0_15px_rgba(248,28,229,0.2)]">{activeOffer.gameId ? <LightboxImage src={`https://assetsdelivery.eldorado.gg/v7/_assets_/icons/v28/${activeOffer.gameId}.png`} alt="Game" className="h-4 w-4 shrink-0 rounded-sm object-cover" /> : <PackageIcon className="text-primary h-4 w-4" />}</div>
                                            ...
                                            <DetailSection title="Images" icon={<ImageIcon className="text-primary h-4 w-4" />}>
                                                {activeOffer.mainOfferImage?.smallImage ? (
                                                    <div className="group/item flex flex-col gap-2 overflow-hidden rounded-lg border border-zinc-800/50 bg-zinc-900/40 p-2.5 sm:col-span-2">
                                                        ...
                                                        <LightboxImage src={`https://fileserviceusprod.blob.core.windows.net/offerimages/${activeOffer.mainOfferImage.smallImage}`} alt="Main" className="h-24 w-24 rounded object-cover" />
                                                    </div>
                                                ) : (
