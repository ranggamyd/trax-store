...
export default function OrderList({ activeOrderList, activeOrderId, setActiveOrderId, isLoadingOrders, fetchOrders, apiError, searchInput, setSearchInput, handleSearchSubmit, openFilter, setOpenFilter, orderStateFilter, setOrderStateFilter, handleScroll, isFetchingNextPage, hasNextPage, chatPreviews = {} }) {
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <div className="flex items-center justify-between rounded border border-zinc-800/50 bg-zinc-950/50 p-1.5">
                                            <div className="flex items-center gap-2 truncate text-sm font-medium text-zinc-300">
                                                {order.raw?.orderOfferDetails?.mainOfferImage?.smallImage ? <LightboxImage src={`https://assetsdelivery.eldorado.gg/v7/_offers-v2_/${order.raw.orderOfferDetails.mainOfferImage.smallImage}`} alt="Item" className="h-6 w-6 shrink-0 rounded object-cover" /> : <Gamepad2Icon className="h-4 w-4 shrink-0 text-zinc-500" />}
                                                <Popover>
                                                    <PopoverTrigger asChild>
                                                        <button type="button" className="flex cursor-pointer flex-col truncate text-left transition-colors hover:text-white" title={order.game || order.gameName} onClick={(e) => e.stopPropagation()}>
                                                            <span className="flex items-center gap-1.5 text-[10px] font-bold tracking-wider text-zinc-400 uppercase">
                                                                {(order.raw?.orderOfferDetails?.gameId || order.raw?.gameId) && <LightboxImage src={`https://assetsdelivery.eldorado.gg/v7/_assets_/icons/v28/${order.raw?.orderOfferDetails?.gameId || order.raw?.gameId}.png`} alt="Game" className="h-3.5 w-3.5 rounded-sm object-cover opacity-90" />}
                                                    ...
                                                    <PopoverContent className="z-50 flex w-auto flex-col gap-2 border-zinc-700 bg-zinc-900 p-3 shadow-xl" onClick={(e) => e.stopPropagation()}>
                                                        <div className="flex flex-col gap-1">
                                                            <span className="flex items-center gap-1.5 text-xs font-bold text-zinc-400 uppercase">
                                                                {(order.raw?.orderOfferDetails?.gameId || order.raw?.gameId) && <LightboxImage src={`https://assetsdelivery.eldorado.gg/v7/_assets_/icons/v28/${order.raw?.orderOfferDetails?.gameId || order.raw?.gameId}.png`} alt="Game" className="h-4 w-4 rounded-sm object-cover opacity-90" />}
                                                                {getGameName(order.raw?.orderOfferDetails?.gameId || order.raw?.gameId) || "Game"}
                                                            </span>
                                                            ...
                                                            {order.raw?.orderOfferDetails?.description && <div className="custom-scrollbar mt-2 max-h-40 overflow-y-auto rounded bg-zinc-950/50 p-2 text-xs whitespace-pre-wrap text-zinc-400">{order.raw.orderOfferDetails.description}</div>}
                                                        </div>
                                                    </PopoverContent>
                                                </Popover>
                                            </div>
