...
export const accountSchema = z.object({
    ...
    // Dulu namanya `is_empty_robux`, padahal artinya KEBALIKANNYA: true -> status
    // ACTIVE (robux ADA). Nama yang bohong kayak gini cepat atau lambat kepakai
    // sesuai namanya sama orang berikutnya, dan datanya kebalik.
    has_robux: z.boolean().default(false).optional(),
});
...
export const templateSchema = z
    ...
    .superRefine((data, ctx) => {
        if (data.type === "Specific" && !data.game_id) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["game_id"], message: "Template Specific wajib pilih game dulu bro!" });
        }
    })
    .transform((data) => (data.type === "Specific" ? data : { ...data, game_id: null, account_id: null }));
