 ...
 *
 * Versi lama di app/notifications/page.js outputnya bahasa Inggris —
 * "Just now", "5m ago", "3d ago" — nyempil di UI yang seluruhnya bahasa
 * Indonesia. Satu-satunya tempat di app ini yang begitu.
 *
 * Sengaja dihitung DI KLIEN: "5 menit lalu" yang dirender server bakal langsung
 * basi, dan makin lama halaman kebuka makin ngawur.
 */
export function formatRelativeTimeId(value) {
    if (!value) return "";

    const seconds = Math.floor((Date.now() - new Date(value).getTime()) / 1000);

    if (seconds < 45) return "baru aja";
